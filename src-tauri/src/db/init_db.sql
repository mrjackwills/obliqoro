...
CREATE TABLE IF NOT EXISTS settings (
	...
	short_break_as_sec INTEGER CHECK(
		short_break_as_sec >= 10
		AND short_break_as_sec <= 120
	) NOT NULL,
	long_break_as_sec INTEGER CHECK(
		long_break_as_sec >= 60
		AND long_break_as_sec <= 600
	) NOT NULL,
	session_as_sec INTEGER CHECK(
		session_as_sec >= 60
		AND session_as_sec <= 3540
	) NOT NULL,
	number_session_before_break INTEGER CHECK(
		number_session_before_break >= 2
		AND number_session_before_break <= 10
	) NOT NULL,
