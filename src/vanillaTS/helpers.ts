...
export function sec_to_minutes (s: number): string {
	const second = Math.trunc(s % 60)
	const minute = Math.floor(s / 60 % 60)
	return minute > 0 ? `${zeroPad(minute)} minute${minute > 1 ? 's' : ''}, ${zeroPad(second)} seconds` : `${zeroPad(second)} second${second > 1 ? 's' : ''}`
}
 ...
 */
export function secondsToText (s: number, hide_second: boolean): string {
	const second = Math.trunc(s % 60)
